...
IAAAAAAAsDyZDwU=`
	...
	for _, tc := range cases {
		...
		checkVolumeContents(t, targetDir, tc.name, tc.payload)
	}
}
...
func TestUpdate(t *testing.T) {
	...
	for _, tc := range cases {
		...
		checkVolumeContents(t, targetDir, tc.name, tc.first)
		if !tc.shouldWrite {
			continue
		}
		...
		checkVolumeContents(t, targetDir, tc.name, tc.next)
	}
}
...
func TestMultipleUpdates(t *testing.T) {
	...
	for _, tc := range cases {
		...
		for _, payload := range tc.payloads {
			...
			checkVolumeContents(t, targetDir, tc.name, payload)
		}
	}
}

func checkVolumeContents(
	t *testing.T,
	targetDir, tcName string,
	payload map[string]FileProjection,
) {
	t.Helper()
...
func TestValidatePayload(t *testing.T) {
	...
	for _, tc := range cases {
		realPayload, err := validatePayload(tc.payload)
		if !tc.valid && err == nil {
			t.Errorf("%v: unexpected success", tc.name)
		}
		...
		if tc.valid {
			...
			realPaths := getPayloadPaths(realPayload)
			if !realPaths.Equal(tc.expected) {
				t.Errorf(
					"%v: unexpected payload paths: %v is not equal to %v",
					tc.name,
					realPaths,
					tc.expected,
				)
	...
	for _, tc := range cases {
		...
		for subpath, expectedDest := range tc.expected {
			...
			if expectedDest != destination {
				t.Fatalf(
					"%v: symlink destination %q not same with expected data dir %q",
					tc.name,
					destination,
					expectedDest,
				)
